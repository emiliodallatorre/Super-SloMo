...
from src.platform_helper import get_torch_backend
...
def extract_frames(video, outDir):
    ...
    retn = os.system(
        '{} -i "{}" -vsync 0 {}/%06d.png'.format(ffmpeg_path, video, outDir)
    )
    if retn:
...
def create_video(dir):
    ...
    print(
        '{} -r {} -i {}/%d.png -c:v libx264 -preset slow -crf 12 -pix_fmt yuv420p -profile:v high -level 4.0 -movflags +faststart "{}"'.format(
            ffmpeg_path, args.fps, dir, args.output
        )
    )
    retn = os.system(
        '{} -r {} -i {}/%d.png -c:v libx264 -preset slow -crf 12 -pix_fmt yuv420p -profile:v high -level 4.0 -movflags +faststart "{}"'.format(
